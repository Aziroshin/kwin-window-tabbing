...
- Window resizing and border toggling gets applied to all grouped windows in unison.
...
- The caption and icon of the current tab bar dont reflect the group's current top window.
...
You can group windows together and have them decorated with a tab bar by using the grouping shortcut (default: `Meta+Space`) to pick the currently focused window (the "tabbee") to be grouped to another window (the "target"). You do this by first focusing the tabbee and hitting the shortcut, then focusing the target window and hitting the shortcut again. If the target window is already in a group, the tabbee will get added to it.
Currently, if you want to abort that process midway, you'll have to make sure the tabbee is focused and hit the grouping shortcut again.
...
- Even though the KWin script and the companion program don't need npm, this repo and its packaging and install procedures are structured around it.
    - This project has an `.nvmrc` file, so you can just run `nvm use` if you have [nvm](https://github.com/nvm-sh/nvm).
    - On debian, you can run `apt-get install npm `.
- You will need the dbus development files and, for the script providing the visible tab bar, python3 (with the "venv" module).
    - Install command for debian based systems (assuming python3 is already installed): `sudo apt-get install libdbus-1-dev python3-venv`.
- Run `npm i` to install the required dependencies.
    - Refer to [package.json](package.json) to see the list of dependencies it'll install.

### Set up the "tab_bar"
The KWin script will work without this, but in order to see anything, you need a companion program that the KWin script communicates with which renders visible tab bars for grouped windows. Currently, [py_tab_bar](tab_bars/py_tab_bar) serves this purpose. Steps to set it up:
- `cd tab_bars/py_tab_bar`.
- `python3 -m venv venv`.
- `. ./venv/bin/activate`.
- `pip install -r requirements.txt`.
    - Refer to [requirements.txt](tab_bars/py_tab_bar/requirements.txt) to see the list of dependencies it'll install.
